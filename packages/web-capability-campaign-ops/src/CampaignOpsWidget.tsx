...
export function CampaignOpsWidget(_props: { node: SchemaNode }) {
  ...
  const submit = async () => {
    ...
    try {
      await apiFetch('/api/v1/campaign-ops/records', token, {
        method: 'POST',
        body: JSON.stringify({ category: 'plan', title: (values.title || '').trim(), channel: (values.channel || '').trim(), metric: (values.metric || '').trim(), note: '', app_public_id: appId || '' }),
      })
  ...
  return (
    <div>
      <h4 style={{ margin: '0 0 8px', fontSize: 14 }}>活动运营</h4>
      <div style={{ display: 'grid', gap: 8, marginBottom: 12 }}>
        <input className="input" placeholder="活动名称" value={values.title || ''} onChange={(e) => setValues((p) => ({ ...p, title: e.target.value }))} />
        <input className="input" placeholder="渠道" value={values.channel || ''} onChange={(e) => setValues((p) => ({ ...p, channel: e.target.value }))} />
        <input className="input" placeholder="目标指标" value={values.metric || ''} onChange={(e) => setValues((p) => ({ ...p, metric: e.target.value }))} />
        ...
        {active.map((t) => {
          ...
          return (
            <li key={t.id} className="list-card">
              ...
              <p className="muted" style={{ margin: '6px 0 0' }}>{t.channel}</p>
              ...
              {t.status === 'open' && (
                <button type="button" className="btn" style={{ background: accent, marginTop: 8 }} onClick={() => void advance(t.id, 'running')}>上线活动</button>
              )}
              {t.status === 'running' && (
                <button type="button" className="btn" style={{ background: accent, marginTop: 8 }} onClick={() => void advance(t.id, 'closed')}>关闭</button>
              )}
            </li>
          )
        })}
