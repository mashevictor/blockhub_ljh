...
from app.services import deco_material_store as store

router = APIRouter(prefix="/deco-material", tags=["deco-material"])
...
@router.post("/records/{record_id}/accepted")
def accepted_api(record_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    item = store.mark_accepted(db, user.tenant_id, record_id)
    if not item:
