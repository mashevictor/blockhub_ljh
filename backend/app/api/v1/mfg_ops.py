"""CapShip · 制造场景专用 API（OEE/领料/保养/排班/能耗/培训）。"""
...
from app.services import mfg_ops_store as store

router = APIRouter(prefix="/mfg-ops", tags=["mfg-ops"])
...
def list_api(
    ...
    if kind not in store.KINDS:
        raise HTTPException(status_code=404, detail="未知制造场景能力")
    items = store.list_records(db, user.tenant_id, kind=kind, app_public_id=app_id or None, status=status)
