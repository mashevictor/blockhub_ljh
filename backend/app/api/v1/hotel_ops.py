"""CapShip · 酒店餐饮共享记录（客诉/食材/卫生/客房服务/宴会/营收等） API。"""
...
from app.services import hotel_ops_store as store

router = APIRouter(prefix="/hotel-ops", tags=["hotel-ops"])
...
def list_api(
    ...
    if kind not in store.KINDS:
        raise HTTPException(status_code=404, detail="未知酒店餐饮能力")
