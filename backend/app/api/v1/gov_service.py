...
from app.services import gov_service_store as store

router = APIRouter(prefix="/gov-service", tags=["gov-service"])
...
@router.post("/records/{record_id}/processing")
def processing_api(record_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    item = store.mark_processing(db, user.tenant_id, record_id)
    if not item:
