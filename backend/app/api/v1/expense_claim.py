...
from app.services import expense_claim_store as store

router = APIRouter(prefix="/expense-claim", tags=["expense-claim"])
...
def create_api(body: CreateBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    if not body.title.strip() or not body.amount.strip():
...
@router.post("/records/{record_id}/paid")
def paid_api(record_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    item = store.mark_paid(db, user.tenant_id, record_id)
    if not item:
        raise HTTPException(status_code=404, detail="记录不存在")
    return {"success": True, "record": item}
