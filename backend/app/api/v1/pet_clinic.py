...
from app.services import pet_clinic_store as store

router = APIRouter(prefix="/pet-clinic", tags=["pet-clinic"])
...
def create_api(body: CreateBody, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    if not body.pet_name.strip() or not body.symptom.strip():
...
@router.post("/records/{record_id}/scheduled")
def scheduled_api(record_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> dict:
    item = store.mark_scheduled(db, user.tenant_id, record_id)
    if not item:
