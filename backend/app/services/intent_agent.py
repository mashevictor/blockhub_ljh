...
def _build_system_prompt(extra_industries: str = "", extra_caps: str = "") -> str:
    ...
    return (
        ...
        "匹配规则（高匹配优先）：\n"
        "- 若用户描述接近首页弹幕场景，必须优先选用「弹幕场景映射」中的 module key（如设备报修→device_repair，质检→quality_inspect，盘点→inventory_count，会员→member_loyalty，导诊→med_triage，护士排班→nurse_shift，玩家FAQ→game_support，家校通知→school_notice，作业答疑→homework_qa，物业报修→property_repair，巡检→site_patrol，课表→class_schedule，酒店预订→hotel_booking），不要用旧的 approval_flow 顶替。\n"
