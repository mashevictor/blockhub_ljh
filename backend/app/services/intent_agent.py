...
def _build_system_prompt(extra_industries: str = "", extra_caps: str = "") -> str:
    ...
    return (
        ...
        "匹配规则（高匹配优先）：\n"
        "- 若用户描述接近首页弹幕场景，必须优先选用「弹幕场景映射」中的 module key（如设备报修→device_repair，质检→quality_inspect，盘点→inventory_count，会员→member_loyalty，导诊→med_triage，护士排班→nurse_shift，玩家FAQ→game_support，家校通知→school_notice，作业答疑→homework_qa，物业报修→property_repair，巡检→site_patrol，课表→class_schedule，酒店预订→hotel_booking，课本学习/家默/学习进度→study_coach，经营看板→ops_kpi，报价合同→quote_contract，销售线索→sales_lead，招聘入职→hire_onboard，制度问答→policy_qa，报销记账→expense_claim，请假审批→leave_request，旅行攻略→travel_plan，法务合同→legal_case，政务办事→gov_service，宠物问诊→pet_clinic，装修选材→deco_material，婚礼筹备→wedding_plan，健身打卡→fitness_checkin，活动运营→campaign_ops，看房签约→house_viewing，外卖配送→delivery_order，KYC/适当性→finance_kyc，反洗钱/风控预警→finance_aml，授信/贷后→credit_approval，尽调/投后→due_diligence，监管报送→regulatory_report，核保理赔→insurance_case），不要用旧的 approval_flow 顶替。\n"
