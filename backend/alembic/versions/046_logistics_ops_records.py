"""logistics_ops_records: 运单/仓配/调度/签收/冷链等共享工单.
...
def upgrade() -> None:
    ...
    for name, cols in (
        ("ix_logistics_ops_records_tenant_id", ["tenant_id"]),
        ("ix_logistics_ops_records_app_public_id", ["app_public_id"]),
        ("ix_logistics_ops_records_reporter_id", ["reporter_id"]),
        ("ix_logistics_ops_records_record_no", ["record_no"]),
        ("ix_logistics_ops_records_kind", ["kind"]),
        ("ix_logistics_ops_records_status", ["status"]),
    ):
        create_index_if_missing(name, "logistics_ops_records", cols)


def downgrade() -> None:
    op.drop_table("logistics_ops_records")
