"""batch CapShip mfg ops: oee/material/maintain/shift/energy/training.
...
def upgrade() -> None:
    ...
    for name, cols in (
        ("ix_mfg_ops_records_tenant_id", ["tenant_id"]),
        ("ix_mfg_ops_records_app_public_id", ["app_public_id"]),
        ("ix_mfg_ops_records_reporter_id", ["reporter_id"]),
        ("ix_mfg_ops_records_record_no", ["record_no"]),
        ("ix_mfg_ops_records_kind", ["kind"]),
        ("ix_mfg_ops_records_status", ["status"]),
    ):
        create_index_if_missing(name, "mfg_ops_records", cols)


def downgrade() -> None:
    op.drop_table("mfg_ops_records")
