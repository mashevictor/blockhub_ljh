...
const PACK_CAPSHIP: Record<string, string[]> = {
  ...
  retail: ['stock_alert', 'store_transfer', 'loss_shrinkage', 'omni_pickup', 'promo_coupon', 'gift_card', 'competitor_price', 'new_sku_launch', 'vip_hold', 'receipt_audit', 'online_refund', 'retail_order', 'return_exchange', 'inventory_count', 'member_loyalty', 'campaign_ops', 'site_patrol', 'delivery_order', 'price_change', 'supplier_recon', 'display_check', 'shelf_replenish', 'pos_exception', 'notify_im', 'approval_flow', 'chart_dashboard', 'kb_document', 'ops_kpi', 'data_nl_query'],
  edu: ['school_notice', 'homework_qa', 'class_schedule', 'study_coach', 'edu_grade_alert', 'edu_tuition', 'edu_attendance', 'edu_quiz', 'edu_textbook', 'chat_qa', 'notify_im', 'kb_document'],
  ...
  hotel: ['hotel_booking', 'room_status', 'hk_task', 'minibar_charge', 'concierge_req', 'group_checkin', 'night_audit', 'table_reserve', 'menu_86', 'kitchen_waste', 'allergen_note', 'guest_complaint', 'food_purchase', 'hygiene_check', 'room_service', 'banquet_order', 'hotel_revenue', 'fnb_order', 'lost_found', 'shift_attendance', 'site_patrol', 'member_loyalty', 'inventory_count', 'approval_flow', 'notify_im', 'chart_dashboard', 'kb_document', 'ops_kpi', 'data_nl_query'],
  energy: ['site_patrol', 'device_repair', 'energy_defect', 'energy_ticket', 'energy_spare', 'energy_emissions', 'energy_outage', 'approval_flow', 'chart_dashboard', 'notify_im', 'kb_document'],
  gov: ['gov_service', 'gov_appeal', 'gov_grid', 'gov_license', 'gov_hotline', 'chat_qa', 'kb_document', 'approval_flow', 'chart_dashboard', 'notify_im'],
  legal: ['legal_case', 'legal_filing', 'legal_evidence', 'legal_hearing', 'legal_contract_ops', 'kb_document', 'chat_qa', 'approval_flow', 'notify_im'],
  hr: ['hire_onboard', 'leave_request', 'policy_qa', 'hr_perf', 'hr_training', 'hr_headcount', 'hr_payroll', 'approval_flow', 'kb_document', 'notify_im'],
  marketing: ['campaign_ops', 'mkt_lead', 'mkt_content', 'sales_lead', 'chart_funnel', 'approval_flow', 'notify_im', 'chat_qa'],
  construction: ['site_patrol', 'deco_material', 'const_safety', 'const_accept', 'const_progress', 'approval_flow', 'notify_im', 'kb_document', 'chart_dashboard'],
  agriculture: ['agro_patrol', 'agro_subsidy', 'agro_inventory', 'kb_document', 'approval_flow', 'notify_im', 'chart_dashboard', 'chat_qa'],
  media: ['campaign_ops', 'media_review', 'media_calendar', 'kb_document', 'approval_flow', 'notify_im', 'chat_qa', 'chart_dashboard'],
  auto: ['auto_service', 'auto_fleet', 'device_repair', 'sales_lead', 'chat_qa', 'approval_flow', 'notify_im', 'chart_dashboard'],
}
...
const PACK_COPY: Record<string, { overview: string; highlights: string[]; tipScenes: Array<{ name: string; tip: string }> }> = {
  ...
  retail: {
    overview: '零售电商深度包覆盖全渠道履约、店仓调拨、券核销储值、竞品采价与售后稽核；正式能力接真 API，空库空列表。',
    highlights: ['店仓调拨与报损', '全渠道自提/仅退款', '券核销与储值卡', '零售经营真聚合'],
    tipScenes: [
      { name: '门店调拨', tip: '店间调拨；store_transfer 真表。' },
      { name: '全渠道自提', tip: '提货码核销；omni_pickup。' },
      { name: '优惠券核销', tip: '门店券码；promo_coupon。' },
      { name: '电商仅退款', tip: '平台退款；online_refund。' },
    ],
  },
  edu: {
  ...
  hotel: {
    overview: '酒店餐饮深度包覆盖房态打扫、礼宾夜审、餐厅订位沽清与过敏原工单；正式能力接真 API，空库空列表。',
    highlights: ['房态/打扫真表', '订位与沽清', '礼宾/夜审', '客房·餐饮双轨'],
    tipScenes: [
      { name: '房态变更', tip: '净房脏房；room_status。' },
      { name: '客房打扫', tip: '急扫任务；hk_task。' },
      { name: '餐厅订位', tip: '人数时段；table_reserve。' },
      { name: '菜品沽清', tip: '档口沽清；menu_86。' },
    ],
  },
  energy: {
