import { adminLoginUrl, adminHomeUrl } from './brand'

/** 生产环境同域 /admin/login；本地 dev 走 5174 */
export function getAdminUrl(): string {
  if (import.meta.env.VITE_ADMIN_URL) return import.meta.env.VITE_ADMIN_URL
  if (typeof window !== 'undefined') return adminLoginUrl()
  return '/admin/login'
}

/** 登录后进入管理后台工作台 */
export function getAdminDashboardUrl(): string {
  if (typeof window !== 'undefined') return adminHomeUrl()
  return '/admin/'
}

/** @deprecated 请用 getAdminUrl()，避免生产构建仍指向 127.0.0.1 */
export const ADMIN_URL = getAdminUrl()
export const PUBLIC_BASE_URL = import.meta.env.VITE_PUBLIC_BASE_URL || 'https://blockhub.club'

export { INDUSTRIES_SHOWCASE as INDUSTRIES } from './showcase'

export const PROMPT_CHIPS = [
  '制造业设备报修 + SOP 问答',
  '销售团队 CRM + 审批 + 漏斗看板',
  '医院内部制度问答 + 排班申请',
  '游戏玩家 FAQ + 客服工单',
  '全员请假报销 + 知识库 + 企微通知',
]

export const MODULES = [
  { cat: '智能交互', items: [
    { key: 'chat_qa', name: '智能问答', icon: '💬' },
    { key: 'chat_voice', name: '语音问答', icon: '🎤' },
    { key: 'shanghai_voice', name: '上海话语音', icon: '🎙️' },
    { key: 'multi_agent', name: '多助手', icon: '🤖' },
  ]},
  { cat: '流程审批', items: [
    { key: 'device_repair', name: '设备报修', icon: '🔧' },
    { key: 'quality_inspect', name: '质检SOP', icon: '✅' },
    { key: 'inventory_count', name: '库存盘点', icon: '📦' },
    { key: 'member_loyalty', name: '会员营销', icon: '🎁' },
    { key: 'med_triage', name: '医疗导诊', icon: '🏥' },
    { key: 'nurse_shift', name: '护士排班', icon: '🗓️' },
    { key: 'game_support', name: '玩家FAQ', icon: '🎮' },
    { key: 'school_notice', name: '家校通知', icon: '📢' },
    { key: 'homework_qa', name: '作业答疑', icon: '📝' },
    { key: 'class_schedule', name: '课表查询', icon: '📅' },
    { key: 'delivery_order', name: '外卖配送', icon: '🚚' },
    { key: 'house_viewing', name: '看房签约', icon: '🏡' },
    { key: 'campaign_ops', name: '活动运营', icon: '📣' },
    { key: 'fitness_checkin', name: '健身打卡', icon: '💪' },
    { key: 'wedding_plan', name: '婚礼筹备', icon: '💒' },
    { key: 'deco_material', name: '装修选材', icon: '🛠️' },
    { key: 'pet_clinic', name: '宠物问诊', icon: '🐾' },
    { key: 'gov_service', name: '政务办事', icon: '🏛️' },
    { key: 'leave_request', name: '请假审批', icon: '📅' },
    { key: 'expense_claim', name: '报销记账', icon: '🧾' },
    { key: 'policy_qa', name: '制度问答', icon: '📘' },
    { key: 'hire_onboard', name: '招聘入职', icon: '🧑‍💼' },
    { key: 'sales_lead', name: '销售线索', icon: '🔥' },
    { key: 'quote_contract', name: '报价合同', icon: '📝' },
    { key: 'ops_kpi', name: '经营看板', icon: '📊' },
    { key: 'legal_case', name: '法务合同', icon: '⚖️' },
    { key: 'travel_plan', name: '旅行攻略', icon: '🧳' },
    { key: 'study_coach', name: '课本学习', icon: '📖' },
    { key: 'property_repair', name: '物业报修', icon: '🏠' },
    { key: 'site_patrol', name: '巡检打卡', icon: '🦺' },
    { key: 'hotel_booking', name: '酒店预订', icon: '🏨' },
    { key: 'approval_flow', name: '审批流', icon: '✅' },
    { key: 'approval_inbox', name: '待办中心', icon: '📥' },
    { key: 'approval_countersign', name: '会签', icon: '✍' },
  ]},
  { cat: '知识数据', items: [
    { key: 'kb_document', name: '知识库', icon: '📚' },
    { key: 'data_nl_query', name: '智能问数', icon: '🔍' },
  ]},
  { cat: '可视化', items: [
    { key: 'chart_dashboard', name: '数据看板', icon: '📊' },
    { key: 'chart_funnel', name: '销售漏斗', icon: '📉' },
  ]},
  { cat: 'Flutter工具', items: [
    { key: 'schedule_alarm', name: '定时闹钟', icon: '⏰' },
    { key: 'flutter_push', name: '移动推送', icon: '📲' },
    { key: 'flutter_scan_qr', name: '扫码识别', icon: '📷' },
    { key: 'flutter_geolocation', name: '定位签到', icon: '📍' },
    { key: 'flutter_camera', name: '拍照上传', icon: '📸' },
    { key: 'flutter_map', name: '地图导航', icon: '🗺️' },
    { key: 'flutter_offline', name: '离线缓存', icon: '💾' },
    { key: 'flutter_biometric', name: '生物识别', icon: '🔒' },
    { key: 'flutter_signature', name: '手写签名', icon: '✍️' },
    { key: 'flutter_speech', name: '语音交互', icon: '🎤' },
    { key: 'flutter_file_picker', name: '文件选择', icon: '📁' },
    { key: 'flutter_pdf', name: 'PDF预览', icon: '📄' },
    { key: 'flutter_webview', name: '内嵌网页', icon: '🌐' },
    { key: 'flutter_chart', name: '移动图表', icon: '📊' },
  ]},
  { cat: '通知集成', items: [
    { key: 'notify_inapp', name: '站内信', icon: '🔔' },
    { key: 'notify_im', name: '企微钉钉', icon: '💼' },
  ]},
  { cat: '权限/外部', items: [
    { key: 'rbac_page', name: '角色权限', icon: '🔐' },
    { key: 'erp_connector', name: 'ERP 对接', icon: '🔌' },
  ]},
]

export const SCENES: Record<string, string[]> = {
  office: ['制度政策问答','请假申请','加班申请','出差申请','报销审批','入职办理','离职交接','用印申请','会议室预约','考勤查询','福利政策咨询','员工手册问答','费用报销','借款申请','合同审批','合同电子签章','发票核验','预算查询','付款申请','法务咨询问答','合规制度库','审计资料检索','制度文档库','SOP作业指导','培训资料库','项目文档共享','会议纪要检索','新人onboarding','内部FAQ','最佳实践库','通用审批','多级会签','待办中心','已办查询','代理审批','超时催办','审批统计','条件分支','部门看板','考勤统计','审批效率','费用汇总','自定义报表','定时推送','数据导出Excel','自然语言查数','审批提醒','公告推送','待办@提醒','邮件/短信','企微/钉钉','订阅消息','到期提醒','IT报障','账号权限申请','软件安装申请','资产领用','资产盘点','网络/VPN申请','IT知识库','对接SAP/用友','对接OA','对接CRM','对接HR系统','单点登录SSO','数据双向同步'],
  mfg: ['设备报修','SOP/工艺问答','生产日报/OEE','质检审批','物料领用','安环隐患上报','排班/考勤','保养计划提醒','图纸/BOM检索','制造·工艺SOP与作业指导库','制造·质检与安环知识库','对接MES/ERP','能耗/碳排统计','技能培训记录'],
  sales: ['线索录入','线索分配','线索清洗','公海领取','线索评分','渠道来源分析','竞品线索监控','转介绍线索','拜访纪要','电话跟进','商机阶段','丢单原因','赢单复盘','客户画像','决策链','跟进任务','新建商机','标准报价','特价折扣','方案报价','竞标报价','报价版本','商机预测','交叉销售','销售合同审批','合同变更申请','回款计划登记','销售开票申请','客户对账单','应收催收提醒','履约节点跟踪','回款确认登记','产品话术库','竞品对比库','成功案例库','解决方案库','销售FAQ','销售培训','产品演示预约','样品礼品申请','销售漏斗分析','业绩排行看板','提成核算查询','区域销售对比','产品线销售分析','销售预测填报','销售问数机器人','目标达成看板','外勤签到','拜访路线','陪访登记','门店巡访','会销活动','商机到期提醒','客户招待申请','联合拜访','Salesforce线索同步','纷享销客商机回写','销售易线索自动导入','企微销售提醒','钉钉销售提醒','飞书销售提醒','CRM线索状态同步','CRM商机阶段回写'],
  med: ['内部制度/合规问答','排班/调班申请','耗材/设备申购','患者宣教资料','不良事件上报','智能导诊(对外)','会诊/转诊申请'],
  game: ['玩家FAQ','客服工单','活动规则检索','版号合规检索','游戏·玩家FAQ与活动规则库','游戏·版号合规与内容审核库','活动上线通知','外包验收审批','版号合规审查','留存运营看板','渠道投放分析','对接游戏后台','2048小游戏','公会举报处理'],
}

export type ViewMode = 'prompt' | 'industry' | 'module'
export type Audience = 'b' | 'c' | 'both'
export type DeliverTarget = 'web' | 'app' | 'both'

export interface PublishResult {
  webUrl: string
  appQr: string
  downloadUrl?: string
  appName: string
  iconUrl?: string
  primaryColor?: string
  moduleCount: number
  /** 本次应用实际包含的模块/场景（用于发布弹窗展示） */
  modules: PublishedModuleItem[]
  scenarios?: string[]
  appId?: string
  schemaUrl?: string
  source?: string
  deliver?: 'web' | 'app' | 'both' | string
  contactEmail?: string
  emailSent?: boolean
  emailConfigured?: boolean
  /** 服务端 APK 是否已就绪（与 Flutter 构建无关，不阻塞发布） */
  apkReady?: boolean
  /** 本应用唯一 Android applicationId（com.blockhub.app.{public_id}） */
  androidAppId?: string
  /** 超出官方能力时的异步 codegen 任务 */
  codegenJobId?: string
  webTemplateId?: string
  appUiId?: string
  capabilityKeys?: string[]
  /** W3：按勾选组装的 Web/App 包清单 */
  buildManifest?: {
    web_pkgs?: string[]
    flutter_pkgs?: string[]
    capability_keys?: string[]
  }
  /** 阶段1：发布契约对齐反馈 */
  capabilityAssembly?: {
    requested_keys?: string[]
    resolved_keys?: string[]
    dropped_keys?: string[]
    dropped_details?: Array<{ key: string; name: string }>
    scenario_added_keys?: string[]
  }
  pageSchema?: Record<string, unknown>
}

export interface PublishedModuleItem {
  key: string
  label: string
  iconKey: string
  kind: 'module' | 'capability' | 'scenario' | 'industry' | 'office'
  source?: 'user' | 'auto' | 'suggest' | 'suggest'
}
